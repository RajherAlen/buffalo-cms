...
export const BenefitBlock: React.FC<PlanningBlockProps> = ({ benefitTitle, benefits, benefitLayout, benefitDescription }) => {
        ...
        <section className='max-w-[1125px] mx-auto py-16'>
            <div className="flex flex-wrap justify-between gap-10">
                <div className='max-w-[457px] flex flex-col justify-between'>
                    {benefitTitle && <RichText data={benefitTitle} className='text-left !text-4xl font-faustina' />}

                    {benefitDescription && <p className="text-gray-700">{benefitDescription}</p>}
                </div>

                <div className="flex flex-col gap-3 max-w-[495px]">
                    {benefits.map((card, idx) => (
                        <div key={idx} className="flex gap-2">
                            <CheckIcon className='mt-1 min-w-5' />
